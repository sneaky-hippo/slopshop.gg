...
module.exports = function mountAgent(app, allHandlers, API_DEFS, db, apiKeys, auth) {
  ...
  const DIRECT_ROUTES = {
    ...
    'unique id': { slug: 'crypto-uuid', inputMap: () => ({}) },
    'generate id': { slug: 'crypto-uuid', inputMap: () => ({}) },
    'identifier': { slug: 'crypto-uuid', inputMap: () => ({}) },
    'unique identifier': { slug: 'crypto-uuid', inputMap: () => ({}) },
    ...
    'base64 encode': { slug: 'text-base64-encode', inputMap: t => ({ text: t.replace(/^.*\s*(?:base64\s*encode|base64 decode)\s*(the\s*)?(text|string)?\s*:?\s*/i, '').trim() }) },
